...
/* JADX INFO: loaded from: voxy-server-side-fabric-0.3.0.jar:META-INF/jars/common-0.3.0.jar:dev/xantha/vss/common/processing/DiskReaderDiagnostics.class */
public class DiskReaderDiagnostics {
    ...
    public String formatDiagnostics(int pendingCount) {
        ...
        double avgMs = completed > 0 ? (this.totalReadTimeNanos.get() / completed) / 1000000.0d : 0.0d;
        long saturated = this.saturationCount.get();
        return String.format("submitted=%d, completed=%d, empty=%d, errors=%d, saturated=%d, avg_read=%.1fms, pending=%d", Long.valueOf(this.submittedCount.get()), Long.valueOf(completed), Long.valueOf(this.emptyCount.get()), Long.valueOf(this.errorCount.get()), Long.valueOf(saturated), Double.valueOf(avgMs), Integer.valueOf(pendingCount));
    }
